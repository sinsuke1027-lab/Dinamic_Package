...
# 2.5 シミュレーション・推移予測ロジック用の設定 (app.py用)
SIM_FINAL_SPRINT_DAYS = 14
SIM_MARKUP_STEP_HIGH_PCT = 2.5
SIM_MARKUP_STEP_LOW_PCT = 1.0
SIM_MARKDOWN_STEP_HIGH_PCT = -5.0
SIM_MARKDOWN_STEP_LOW_PCT = -2.0
SIM_FORECAST_BOOST_PCT = 15.0

# 2.6 非線形・動的価格弾力性モデル用設定
SIM_ELASTICITY_PRICE_RATIO_HIGH_THRES = 2.0  # 半額未満（値引しすぎ）のしきい値
SIM_ELASTICITY_PRICE_RATIO_LOW_THRES = 1.0   # 定価以上（値上げ）のしきい値
